...
//#endregion
//#region src/core/types/join.d.ts
...
//#endregion
//#region src/core/types/escape.d.ts
...
interface Input<V extends string, G extends string = never, C extends (string | undefined)[] = []> {
   ...
   */
  and: {
    <I extends InputSource[], CG extends any[] = MapToCapturedGroupsArr<I>>(...inputs: I): Input<`${V}${Join<MapToValues<I>, '', ''>}`, G | MapToGroups<I>, [...C, ...CG]>; /** this adds a new pattern to the current input, with the pattern reference to a named group. */
  ...
  times: {
    <N extends number, NV extends string = IfUnwrapped<V, `(?:${V}){${N}}`, `${V}{${N}}`>>(number: N): Input<NV, G, C>; /** specify that the expression can repeat any number of times, _including none_ */
    any: <NV extends string = IfUnwrapped<V, `(?:${V})*`, `${V}*`>>() => Input<NV, G, C>; /** specify that the expression must occur at least `N` times */
    atLeast: <N extends number, NV extends string = IfUnwrapped<V, `(?:${V}){${N},}`, `${V}{${N},}`>>(number: N) => Input<NV, G, C>; /** specify that the expression must occur at most `N` times */
    atMost: <N extends number, NV extends string = IfUnwrapped<V, `(?:${V}){0,${N}}`, `${V}{0,${N}}`>>(number: N) => Input<NV, G, C>; /** specify a range of times to repeat the previous pattern */
  ...
  as: <K extends string>(key: K) => Input<V extends `(?:${infer S})` ? `(?<${K}>${S})` : `(?<${K}>${V})`, G | K, [V extends `(?:${infer S})` ? `(?<${K}>${S})` : `(?<${K}>${V})`, ...C]>;
  ...
  groupedAs: <K extends string>(key: K) => Input<V extends `(?:${infer S})` ? `(?<${K}>${S})` : `(?<${K}>${V})`, G | K, [V extends `(?:${infer S})` ? `(?<${K}>${S})` : `(?<${K}>${V})`, ...C]>;
  /** this capture the entire input so far as an anonymous group */
  grouped: () => Input<V extends `(?:${infer S})${infer E}` ? `(${S})${E}` : `(${V})`, G, [V extends `(?:${infer S})${'' | '?' | '+' | '*' | `{${string}}`}` ? `(${S})` : `(${V})`, ...C]>;
...
//#endregion
//#region src/core/types/sources.d.ts
type InputSource<S extends string = string, T extends string = never> = S | Input<any, T>;
type MapToValues<T extends InputSource[]> = T extends [infer First, ...infer Rest extends InputSource[]] ? First extends InputSource ? [GetValue<First>, ...MapToValues<Rest>] : [] : [];
type MapToGroups<T extends InputSource[]> = T extends [infer First, ...infer Rest extends InputSource[]] ? First extends Input<any, infer K> ? K | MapToGroups<Rest> : MapToGroups<Rest> : never;
type MapToCapturedGroupsArr<Inputs extends any[], MapToUndefined extends boolean = false, CapturedGroupsArr extends any[] = [], Count extends any[] = []> = Count['length'] extends Inputs['length'] ? CapturedGroupsArr : Inputs[Count['length']] extends Input<any, any, infer CaptureGroups> ? [CaptureGroups] extends [never] ? MapToCapturedGroupsArr<Inputs, MapToUndefined, [...CapturedGroupsArr], [...Count, '']> : MapToUndefined extends true ? MapToCapturedGroupsArr<Inputs, MapToUndefined, [...CapturedGroupsArr, undefined], [...Count, '']> : MapToCapturedGroupsArr<Inputs, MapToUndefined, [...CapturedGroupsArr, ...CaptureGroups], [...Count, '']> : MapToCapturedGroupsArr<Inputs, MapToUndefined, [...CapturedGroupsArr], [...Count, '']>;
//#endregion
//#region src/core/types/magic-regexp.d.ts
...
type MapToStringCapturedBy<Ar extends (string | undefined)[]> = { [K in keyof Ar]: Ar[K] extends string ? StringCapturedBy<Ar[K]> | undefined : undefined };
...
//#endregion
export { caseInsensitive as A, whitespace as C, Join as D, wordChar as E, unicode as F, withIndices as I, global as M, multiline as N, UnionToTuple as O, sticky as P, tab as S, wordBoundary as T, letter as _, InputSource as a, not as b, MapToValues as c, carriageReturn as d, char as f, exactly as g, digit as h, StringCapturedBy as i, dotAll as j, Flag as k, Input as l, charNotIn as m, MagicRegExpMatchArray as n, MapToCapturedGroupsArr as o, charIn as p, MapToStringCapturedBy as r, MapToGroups as s, MagicRegExp as t, anyOf as u, linefeed as v, word as w, oneOrMore as x, maybe as y };