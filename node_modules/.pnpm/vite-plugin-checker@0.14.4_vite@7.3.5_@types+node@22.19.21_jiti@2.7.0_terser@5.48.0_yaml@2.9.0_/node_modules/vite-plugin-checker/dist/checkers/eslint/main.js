		...
		async configureServer({ root }) {
			...
			const handleFileChange = async (filePath, type) => {
				if (!effectiveUseFlatConfig) {
					...
					if (Array.isArray(extensions) && !extensions.includes(extension)) return;
				}
				if (await eslint.isPathIgnored(filePath)) return;
				const absPath = path.resolve(root, filePath);
				if (type === "unlink") manager.updateByFileId(absPath, []);
				else if (type === "change") {
					const newDiagnostics = (await eslint.lintFiles(filePath)).flatMap((d) => normalizeEslintDiagnostic(d));
