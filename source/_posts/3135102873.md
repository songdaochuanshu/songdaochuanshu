...
![image-20220602161518842](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/dd942e1b87b54d1f8144cc8379c33d34~tplv-k3u1fbpfcp-zoom-1.image)

结果如下：

![image-20220602103107720](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/481f8bf5d44e476fac396800c0082595~tplv-k3u1fbpfcp-zoom-1.image)

klog.xxx能直接打印日志的原因
------------------

![image-20220602162933166](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/21afa93e4eb94547a470e121a00c899d~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602164445885](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/1ea06f1e9f814268a5302f55ec607039~tplv-k3u1fbpfcp-zoom-1.image)

可以看到`logger`实例默认的日志打印级别是`LevelInfo`，`klog`通过常量计数器定义了`0～6`种日志级别：

![image-20220602172735438](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/75b89446d5704ab786483d253fc4522a~tplv-k3u1fbpfcp-zoom-1.image)

FullLogger接口
------------

![image-20220602164335103](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ea32e2fdc72945ea8c8b74d027a9bcb7~tplv-k3u1fbpfcp-zoom-1.image)

默认的`logger`实例是通过`defaultLogger`结构初始化的，且`defaultLogger`结构实现了`FullLogger`接口定义的所有函数（接口定义了上面说的三类，每一类7种日志打印函数）

![image-20220602170223047](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/c2733b42145a41f09e6cb37119644edb~tplv-k3u1fbpfcp-zoom-1.image)

并且观察`defaultLogger`结构的属性`stdlog`，就是**Go**原生的日志库`log`定义的`Logger`类型，因此`klog`的所有日志操作，最终都是借助**Go**原生`log`库实现的。

![image-20220602170523008](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/06137ad107414218af85923aa0fa9ed0~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602172144229](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/3d812b7cf6614cb3b2caf58f294b9c7e~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602175219202](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/41df0a8088654ee199dd6ef6d8593015~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602180622096](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/651ddfef0e434ea18d817043dae94c78~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602175507724](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/34fac84dae16403aa2bee12a525429af~tplv-k3u1fbpfcp-zoom-1.image)

定制自己的Logger
-----------

![image-20220602181634895](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/58eaceede6994b2aa4e98472323b673f~tplv-k3u1fbpfcp-zoom-1.image)
...
![image-20220602183649139](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/ae3228b2ece2490aa25bf5a4d2eb61e2~tplv-k3u1fbpfcp-zoom-1.image)

下面修改日志打印级别为`Notice`（高于`Info`），并且重定向日志的输出：

![image-20220602184046958](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/5c00d059c410423494bf04e087ab41ae~tplv-k3u1fbpfcp-zoom-1.image)

这里指定了日志输出到文件`log.txt`中，并且因为`Info`级别低于声明的`Notice`，因此日志输出操作被忽略：

![image-20220602184350533](https://p3-juejin.byteimg.com/tos-cn-i-k3u1fbpfcp/e19a539d9e584e4da064f002567756db~tplv-k3u1fbpfcp-zoom-1.image)
