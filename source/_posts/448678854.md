---
layout: post
title: "快速上手python的简单web框架flask"
date: "2023-02-06T17:14:05.742Z"
---
...
*   [简介](#简介)
*   [web框架的重要组成部分](#web框架的重要组成部分)
*   [快速上手flask](#快速上手flask)
    *   [flask的第一个应用](#flask的第一个应用)
*   [flask中的路由](#flask中的路由)
    *   [不同的http方法](#不同的http方法)
    *   [静态文件](#静态文件)
    *   [使用模板](#使用模板)
...
可以看到IDE为我们设置了几个环境变量，分别是FLASK\_APP：表示要运行的app名称。FLASK\_ENV：表示现在的运行环境是开发环境还是线上环境。FLASK\_DEBUG表示是否是debug模式。
...
这里面我们用到了url\_for这个方法，这个方法实际上是用来构建对应方法的url的，可以举下面的几个例子来对url\_for有个深入的了解。

urL\_for的第一个参数是方法名，后面接的是url中定义的变量，如果url中并没有这个变量，那么将会以参数的形式附加在url的后面：
...
怎么使用模板呢？我们在返回的时候，可以使用render\_template方法：
