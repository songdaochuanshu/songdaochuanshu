---
layout: post
title: "二阶段目标检测网络-Faster RCNN 详解"
date: "2022-12-16T01:16:27.027Z"
---
二阶段目标检测网络-Faster RCNN 详解
========================

![二阶段目标检测网络-Faster RCNN 详解](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194053520-1641431370.png) backbone 为 vgg16 的 faster rcnn 网络结构如下图所示，可以清晰的看到该网络对于一副任意大小 PxQ 的图像，首先缩放至固定大小 MxN，然后将 MxN 图像送入网络。

*   [Faster RCNN 网络概述](#faster-rcnn-%E7%BD%91%E7%BB%9C%E6%A6%82%E8%BF%B0)
*   [Conv layers](#conv-layers)
*   [RPN 网络](#rpn-%E7%BD%91%E7%BB%9C)
    *   [Anchors](#anchors)
    *   [生成 RPN 网络训练集](#%E7%94%9F%E6%88%90-rpn-%E7%BD%91%E7%BB%9C%E8%AE%AD%E7%BB%83%E9%9B%86)
    *   [positive/negative 二分类](#positivenegative-%E4%BA%8C%E5%88%86%E7%B1%BB)
    *   [RPN 生成 RoIs(Proposal Layer)](#rpn-%E7%94%9F%E6%88%90-roisproposal-layer)
    *   [RPN 网络总结](#rpn-%E7%BD%91%E7%BB%9C%E6%80%BB%E7%BB%93)
*   [ROIHead/Fast R-CNN](#roiheadfast-r-cnn)
    *   [Roi pooling](#roi-pooling)
    *   [ROI Head 训练](#roi-head-%E8%AE%AD%E7%BB%83)
    *   [ROI Head 测试](#roi-head-%E6%B5%8B%E8%AF%95)
*   [概念理解](#%E6%A6%82%E5%BF%B5%E7%90%86%E8%A7%A3)
    *   [四类损失](#%E5%9B%9B%E7%B1%BB%E6%8D%9F%E5%A4%B1)
    *   [三个 creator](#%E4%B8%89%E4%B8%AA-creator)
...
![faster-rcnn连接图](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194001322-2111202837.png)
...
![faster-rcnn网络详细结构图](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194001836-139948814.png)
...
![RPN网络结构图](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194002233-29673133.png)
...
在`RPN`中，作者提出了`anchors`，在代码中，anchors 是一组由 generate\_anchors.py 生成的矩形框列表。运行官方代码的 `generate_anchors.py` 可以得到以下示例输出.这里生成的坐标是在原图尺寸上的坐标，在特征图上的一个像素点，可以对应到原图上一个 \\(16\\times 16\\)大小的区域。
...
其中每行的 4 个值 \\((x\_{1}, y\_{1}, x\_{2}, y\_{2})\\) 表矩形左上和右下角点坐标。9 个矩形共有 3 种形状，长宽比为大约为 \\(width:height \\epsilon \\{1:1, 1:2, 2:1\\}\\) 三种，如下图。**实际上通过 anchors 就引入了检测中常用到的多尺度方法**。

![anchor示例](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194002545-1434472369.png)
...
*   对于剩下的 anchor，从中选择和任意一个 gt\_bbox 重叠度超过 `0.7` 的 anchor ，同样作为正样本;特殊情况下，如果正样本不足 `128`(256 的 1/2)，则用负样本凑。
...
在挑选 `1:1` 正负样本比例的 `anchor` 用作 `RPN` 训练集后，还需要计算训练集数据对应的标签。对于每个 `anchor`, 对应的标签是 `gt_label` 和 `gt_loc`。`gt_label` 要么为 `1`（前景），要么为 `0`（背景），而 `gt_loc` 则是由 `4` 个位置参数 \\((t\_x,t\_y,t\_w,t\_h)\\) 组成，它们是 `anchor box` 与 `ground truth bbox` 之间的偏移量，因为回归偏移量比直接回归座标更好。在 `Faster RCNN`原文，`positive anchor` 与 `ground truth` 之间的偏移量 \\((t\_{x}, t\_{y})\\) 与尺度因子 \\((t\_{w}, t\_{h})\\) 计算公式如下:

\\\[\\begin{aligned} t\_{x} = (x-x\_{a})/w\_{a}, t\_{y}=(y-y\_{a})/h\_{a} \\\\ t\_{w} = log(w/w\_{a}), t\_{h}=log(h/h\_{a}) \\\\ t\_{x}^{\\ast } = (x^{\\ast }-x\_{a})/w\_{a}, t\_{y}^{\\ast}=(y^{\\ast}-y\_{a})/h\_{a} \\\\ t\_{w}^{\\ast } = log(w^{\\ast }/w\_{a}), t\_{h}^{\\ast }=log(h^{\\ast }/h\_{a}) \\end{aligned}\\\]

参数解释：where \\(x, y, w,\\) and \\(h\\) denote the box’s center coordinates and its width and height. Variables \\(x, x\_{a}\\)，and \\(x^{\*}\\) are for the predicted box, anchor box, and groundtruth box respectively (likewise for \\(y, w, h\\)).
...
![rpn的loss计算公式](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194002838-1295539543.png)

**公式解释**：Here, \\(i\\) is the index of an anchor in a mini-batch and \\(p\_{i}\\) is the predicted probability of anchor i being an object. The ground-truth label \\(p\_i^{\\ast}\\) is 1 if the anchor is positive, and is 0 if the anchor is negative. \\(t\_{i}\\) is a vector representing the 4 parameterized coordinates of the predicted bounding box, and \\(t\_i^\*\\) is that of theground-truth box associated with a positive anchor.
...
![ROI-Head结构图](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194003238-1327053323.png)
...
![ROI-Align](https://img2023.cnblogs.com/blog/2989634/202212/2989634-20221215194003632-159676206.png)
...
*   对于位置的回归损失，也采用 **Smooth\_L1 Loss**, 只不过**只对正样本计算损失**，而且是只对正样本中的对应类别的 \\(4\\) 个参数计算损失。举例来说：
