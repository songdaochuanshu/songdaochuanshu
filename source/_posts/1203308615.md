...
*   **解耦**：引入消息队列之前，下单完成之后，需要订单服务去调用库存服务减库存，调用营销服务加营销数据……引入消息队列之后，可以把订单完成的消息丢进队列里，下游服务自己去调用就行了，这样就完成了订单服务和其它服务的解耦合。
    ...
    ![消息队列解耦](https://img-blog.csdnimg.cn/img_convert/c1732b358d2b0ab0395d0603ba42a66e.png)
    
*   **异步**：订单支付之后，我们要扣减库存、增加积分、发送消息等等，这样一来这个链路就长了，链路一长，响应时间就变长了。引入消息队列，除了`更新订单状态`，其它的都可以**异步**去做，这样一来就来，就能降低响应时间。
    
    ![消息队列异步](https://img-blog.csdnimg.cn/img_convert/31e5ab7061725d4b105377b72ab438b4.png)
...
*   **削峰**：消息队列合一用来削峰，例如秒杀系统，平时流量很低，但是要做秒杀活动，秒杀的时候流量疯狂怼进来，我们的服务器，Redis，MySQL各自的承受能力都不一样，直接全部流量照单全收肯定有问题啊，严重点可能直接打挂了。
    ...
    ![消息队列削峰](https://img-blog.csdnimg.cn/img_convert/3fb522ba30bad10c0725518775fa9071.png)
...
![四大消息队列对比](https://img-blog.csdnimg.cn/img_convert/cb01f2845809f9818d671efb9ab7014d.png)
...
*   **队列模型**
    ...
    ![队列模型](https://img-blog.csdnimg.cn/img_convert/12dfe4b0589ca6d7addf1b33d92133d4.png)
...
*   **发布/订阅模型**
    ...
    ![发布-订阅模型](https://img-blog.csdnimg.cn/img_convert/71d4d704b1c5efacef14ee2e4f49436e.png)
...
![RocketMQ消息的组成](https://img-blog.csdnimg.cn/img_convert/845bdcebd6f30e6f496d302ed0afbead.png)
...
![RocketMQ消息模型](https://img-blog.csdnimg.cn/26a74f2d0e394a04b9119a560023b9f2.png)
...
![两种消费模式](https://img-blog.csdnimg.cn/img_convert/94a10bdbaa3c3b1b5fd971b7a0b84834.png)
...
![RocketMQ架构](https://img-blog.csdnimg.cn/img_convert/6a09acec017128ee360b2be89ac947c0.png)
...
![RocketMQ类比邮政体系](https://img-blog.csdnimg.cn/img_convert/f9a94ed7734ed92f3661d399e51c7a82.png)
...
![RocketMQ存储-图片来源官网](https://img-blog.csdnimg.cn/img_convert/940e624ce6b99a250589616edfd38f55.png)
...
![消息传递三阶段](https://img-blog.csdnimg.cn/img_convert/f998e17bd2a85275c6fb65648ae109a7.png)
...
![同步刷盘和异步刷盘-图片来源官网](https://img-blog.csdnimg.cn/img_convert/4dcbe10d0e1da3a506abed4e22cc1b8a.png)
...
![消息重复处理](https://img-blog.csdnimg.cn/img_convert/407abade5fc878813fc1b4041f5063eb.png)
...
![消息积压处理](https://img-blog.csdnimg.cn/img_convert/ae9502706195336d67f76d5227cf95b5.png)
...
![消息迁移扩容消费](https://img-blog.csdnimg.cn/321b6d5aba5a49f39d59dde5853b482e.png)
...
![顺序消息](https://img-blog.csdnimg.cn/img_convert/7eef8d1d14795c02c6f8407e1cecc54c.png)
...
![部分顺序消息](https://img-blog.csdnimg.cn/img_convert/0540d4eb74e9357007ceca7876f3b7b7.png)

发送端使用 MessageQueueSelector 类来控制 把消息发往哪个 Message Queue 。

![顺序消息生产-例子来源官方](https://img-blog.csdnimg.cn/img_convert/e2399038c4b28bb99643df8f46de5182.png)

消费端通过使用 MessageListenerOrderly 来解决单 Message Queue 的消息被并发处理的问题。  
![消费端述](https://img-blog.csdnimg.cn/1cdad3c026de470c9b187d65ef0ab0b1.png)
...
![全局顺序消息](https://img-blog.csdnimg.cn/img_convert/4bdc4f11f8c9f9d810b2fddc436b7c8d.png)
...
![消息过滤](https://img-blog.csdnimg.cn/img_convert/41c7a96d0983f31175fab2c3fdfd7259.png)
...
![延迟消息处理流程-图片来源见水印](https://img-blog.csdnimg.cn/img_convert/4c951cb4e02909b956b25b81a2681d6a.png)
...
![RocketMQ实现消息事务](https://img-blog.csdnimg.cn/img_convert/6effabfdb4caab61b453b19b9d86b191.png)
...
![NameServer集群](https://img-blog.csdnimg.cn/0fb79fbebf0a40d39ad55d9ef43e5920.png)

RocketMQ的高可用主要是在体现在Broker的读和写的高可用，Broker的高可用是通过`集群`和`主从`实现的。

![Broker集群、主从示意图](https://img-blog.csdnimg.cn/img_convert/d1c473a4c06aeec24c2d67c82da7a9ef.png)
...
![RocketMQ整体工作流程](https://img-blog.csdnimg.cn/img_convert/c85b524ee71d30ac503553c6b9a35175.png)
...
![消息存储文件](https://img-blog.csdnimg.cn/b93a563a264a43dbb79d0895ad27a696.png)

消息存储的整体的设计：

![消息存储整体设计-来源官网](https://img-blog.csdnimg.cn/28df7dc97aba442a8c1fbbf924a868d0.png)
...
*   **CommitLog**：消息主体以及元数据的存储主体，存储Producer端写入的消息主体内容,消息内容不是定长的。单个文件大小默认1G, 文件名长度为20位，左边补零，剩余为起始偏移量，比如00000000000000000000代表了第一个文件，起始偏移量为0，文件大小为1G=1073741824；当第一个文件写满了，第二个文件为00000000001073741824，起始偏移量为1073741824，以此类推。消息主要是顺序写入日志文件，当文件满了，写入下一个文件。
    ...
    ![CommitLog](https://img-blog.csdnimg.cn/img_convert/5766872961a4613255b5aff8233a23d4.png)
...
*   **ConsumeQueue**：消息消费队列，引入的目的主要是提高消息消费的性能，由于RocketMQ是基于主题topic的订阅模式，消息消费是针对主题进行的，如果要遍历commitlog文件中根据topic检索消息是非常低效的。
    ...
    ![Comsumer Queue](https://img-blog.csdnimg.cn/img_convert/7461a4ed50ad97bcb2ef06cb5b82e357.png)
...
*   **IndexFile**：IndexFile（索引文件）提供了一种可以通过key或时间区间来查询消息的方法。Index文件的存储位置是：\\(HOME \\store\\index\\){fileName}，文件名fileName是以创建时的时间戳命名的，固定的单个IndexFile文件大小约为400M，一个IndexFile可以保存 2000W个索引，IndexFile的底层存储设计为在文件系统中实现HashMap结构，故RocketMQ的索引文件其底层实现为hash索引。
    ...
    ![IndexFile文件示意图-来源参考[2]](https://img-blog.csdnimg.cn/img_convert/baa487901eef64b9d60746de920e1233.png)
...
![总结](https://img-blog.csdnimg.cn/img_convert/ffe2886492241c5b6a0f3fd16bdc6ba5.png)
...
![传统文件传输示意图-来源《图解操作系统》](https://img-blog.csdnimg.cn/img_convert/99b82368493e7ad95b0764d0e09ee17c.png)
...
![mmap示意图-来源《图解操作系统》](https://img-blog.csdnimg.cn/img_convert/9fbc3c199772ad9464a58fd0cf269049.png)
...
![异步刷盘](https://img-blog.csdnimg.cn/img_convert/65ca1a219d00e3a1214bb86d8fa2342a.png)
...
\[外链图片转存失败,源站可能有防盗链机制,建议将图片保存下来直接上传(img-VuMGUv6B-1649246218316)([https://cdn.jsdelivr.net/gh/fighter3/picgo/img/20220405230011.png](https://cdn.jsdelivr.net/gh/fighter3/picgo/img/20220405230011.png))\]
...
![img](https://img-blog.csdnimg.cn/e2795107734045f181721675993ac216.png)
...
![Cosumer分配](https://img-blog.csdnimg.cn/img_convert/56962aee5389037f1ac566a295cc91b3.png)

(4) 然后，调用updateProcessQueueTableInRebalance()方法，具体的做法是，先将分配到的消息队列集合（mqSet）与processQueueTable做一个过滤比对。  
![负载均衡示意图-来源官网](https://img-blog.csdnimg.cn/3a1bb699ba124a52973ffa6eb5af8e20.png?)
...
![长轮询简单示意图](https://img-blog.csdnimg.cn/img_convert/e0e6ed91370300cdd4dff2ee1fbdd512.png)
...
\[5\]. [http://dreamcat.ink/java-interview/docs/knows/classify/dis/RocketMQ/](http://dreamcat.ink/java-interview/docs/knows/classify/dis/RocketMQ/)
...
*   [面渣逆袭：JVM经典五十问，这下面试稳了！  
    ](https://mp.weixin.qq.com/s/XYsEJyIo46jXhHE1sOR_0Q)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/1136185996ca4349ae24d1d985828a1c.png?#pic_center)