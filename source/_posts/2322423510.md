...
![Switch Transformers paper 论文中的 MoE layer](https://devrel.andfun.cn/devrel/posts/2023/12/26/skjNlK.png)
...
![Outrageously Large Neural Network 论文中的 MoE layer](https://devrel.andfun.cn/devrel/posts/2023/12/26/ANmJf4.png)
...
\\\[\\text{KeepTopK}(v, k)\_i = \\begin{cases} v\_i & \\text{if } v\_i \\text{ is in the top } k \\text{ elements of } v, \\\\ -\\infty & \\text{otherwise.} \\end{cases} \\\]
...
![GShard 论文中的 MoE Transformer Encoder](https://devrel.andfun.cn/devrel/posts/2023/12/26/hHo1WL.png)
...
![Switch Transformer 论文中的 Switch Transformer Layer](https://devrel.andfun.cn/devrel/posts/2023/12/26/KXZ9bR.png)
...
![使用混合精度不会降低模型质量并可实现更快的训练](https://devrel.andfun.cn/devrel/posts/2023/12/26/5TBMIY.png)

这个 [Jupyter Notebook](https://colab.research.google.com/drive/1aGGVHZmtKmcNBbAwa9hbu58DDpIuB5O4?usp=sharing) 展示了如何对 Switch Transformers 进行微调以进行摘要生成的详细指南。然而，在开始微调 Switch Transformers 之前，强烈建议您先阅读关于微调混合专家模型部分的内容。
...
![ST-MoE 论文中显示了哪些令牌组被发送给了哪个专家的表格](https://devrel.andfun.cn/devrel/posts/2023/12/26/39LQvI.png)
...
![在小任务 (左图) 中，我们可以看到明显的过拟合，因为稀疏模型在验证集中的表现要差得多。在较大的任务 (右图) 中，MoE 则表现良好。该图来自 ST-MoE 论文](https://devrel.andfun.cn/devrel/posts/2023/12/26/35AOxK.png)
...
![通过仅冻结 MoE 层，我们可以在保持质量的同时加快训练速度。该图来自 ST-MoE 论文](https://devrel.andfun.cn/devrel/posts/2023/12/26/xWWNHF.png)

在微调稀疏混合专家模型 (MoE) 时需要考虑的最后一个问题是，它们有特别的微调超参数设置——例如，稀疏模型往往更适合使用较小的批量大小和较高的学习率，这样可以获得更好的训练效果。

![降低学习率和调大批量可以提升稀疏模型微调质量。该图来自 ST-MoE 论文](https://devrel.andfun.cn/devrel/posts/2023/12/26/UETwAe.png)
...
![与稠密模型相比，稀疏模型从指令微调中受益更多。该图来自 MoEs Meets instructions Tuning 论文](https://devrel.andfun.cn/devrel/posts/2023/12/26/wKz4nG.png)
...
![Switch Transformers 论文中展示如何使用不同的并行技术在节点上分割数据和模型的插图](https://devrel.andfun.cn/devrel/posts/2023/12/26/LSZ6Bc.png)
...
![针对不同规模的专家和令牌数量的块稀疏矩阵乘法。该图来自 MegaBlocks 论文](https://devrel.andfun.cn/devrel/posts/2023/12/26/a3Ixxw.png)
