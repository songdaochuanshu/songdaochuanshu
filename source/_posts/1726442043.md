---
layout: post
title: "深入理解 Python 虚拟机：字典（dict）的优化"
date: "2023-03-26T01:13:34.959Z"
---
深入理解 Python 虚拟机：字典（dict）的优化
===========================

![深入理解 Python 虚拟机：字典（dict）的优化](https://img2023.cnblogs.com/blog/2519003/202303/2519003-20230326013658961-496640853.png) 在前面的文章当中我们讨论的是 python3 当中早期的内嵌数据结构字典的实现，在本篇文章当中主要介绍在后续对于字典的内存优化。
...
*   当哈希表长度小于 0xff 时，dk\_indices 的数据类型为 int8\_t ，即一个元素值占一个字节。
*   当哈希表长度小于 0xffff 时，dk\_indices 的数据类型为 int16\_t ，即一个元素值占 2 一个字节。
*   当哈希表长度小于 0xffffffff 时，dk\_indices 的数据类型为 int32\_t ，即一个元素值占 4 个字节。
*   当哈希表长度大于 0xffffffff 时，dk\_indices 的数据类型为 int64\_t ，即一个元素值占 8 个字节。
