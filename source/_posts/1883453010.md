...
![Screenshot of YAML metadata](https://man-archives.oss-cn-hangzhou.aliyuncs.com/goofan/202401032218922.png)  
_IMDB 数据集的 YAML 元数据部分_

迄今为止，Hub 上数据集上最常见的语言是英语，有大约 19% 的数据集将其语言标注为 `en` (这还没把 `en` 的其他变体统计在内，因此实际百分比可能会比 19% 要高得多)。这个现象符合我们的预期。

![Distribution of language tags](https://man-archives.oss-cn-hangzhou.aliyuncs.com/goofan/202401032218942.png)  
_Hugging Face Hub 上的数据集的频率及占比_

如果排除掉英语，语言分布又如何呢？我们可以看到，有几种语言相对占主导，随后的其他语言的频率则出现了平稳下降。

![Distribution of language tags](https://man-archives.oss-cn-hangzhou.aliyuncs.com/goofan/202401032218702.png)  
_Hub 上数据集的语言标签分布情况 (除英语外)_

这里，我们发现一个重大问题，那就是大多数数据集 (约 87%) 没有指明其所使用的语言，只有大约 13% 的数据集在其元数据中指明了语言信息。

![Barchart](https://man-archives.oss-cn-hangzhou.aliyuncs.com/goofan/202401032219108.png)  
_具有语言元数据的数据集占比。True 表示指明了语言元数据，False 表示未列出语言元数据。No card data 意味着没有任何元数据，抑或是\`huggingface\_hub\` Python 库无法加载它。_
...
![Prediction workflow](https://man-archives.oss-cn-hangzhou.aliyuncs.com/goofan/202401032219494.png)  
_预测过滤流程图_
...
> 英文原文: [https://hf.co/blog/huggy-lingo](https://hf.co/blog/huggy-lingo)
> 
