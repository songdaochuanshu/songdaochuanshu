...
Linux x64通过qemu的rdinit方式调用的C Main，实际上是通过load\_elf\_binary(加载和解析elf)和start\_thread(设置Ip和sp)，用缺页异常来调用。那么Arm32里面是如何调用C Main的呢？本篇看下。
