...
首先，它并不是使用游览器原生的 [Intl](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Intl)，Angular 自己写了一套逻辑 (为什么 Angular 要自己写一套，不使用原生的？我不太清楚，可能是当时 Inlt 支持度还不高？不管怎样，目前我的感觉是，以后 Angular 很可能会改用原生的 Intl)。
