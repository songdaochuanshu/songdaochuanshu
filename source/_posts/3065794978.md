---
layout: post
title: "netty系列之:kequeue传输协议详解"
date: "2022-07-04T09:19:55.278Z"
---
...
*   [KQueueServerSocketChannel和KQueueSocketChannel](#kqueueserversocketchannel和kqueuesocketchannel)
*   [总结](#总结)
...
selectStrategy.calculateStrategy用来判断当前的select状态，默认情况下有三个状态，分别是：SELECT,CONTINUE,BUSY\_WAIT。 这三个状态都是负数：
...
分别表示当前的IO在slect的block状态，或者跳过当前IO的状态，和正在IO loop pull的状态。BUSY\_WAIT是一个非阻塞的IO PULL，kqueue并不支持，所以会fallback到SELECT。
...
> 更多内容请参考 [http://www.flydean.com/53-1-netty-kqueue-transport/](http://www.flydean.com/53-1-netty-kqueue-transport/)
> 
