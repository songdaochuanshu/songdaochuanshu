...
layout: post
title: "ONNX模型分析与使用"
date: "2023-01-09T18:20:49.943Z"
---
ONNX模型分析与使用
===========

![ONNX模型分析与使用](https://img2023.cnblogs.com/blog/2989634/202301/2989634-20230109141805112-1157996171.png) ONNX 的本质只是一套开放的 ML 模型标准，模型文件存储的只是网络的拓扑结构和权重（其实每个深度学习框架最后保存的模型都是类似的），脱离开框架是没办法对模型直接进行 inference的。
...
The name of the operator, as used in graph nodes. MUST be unique within the operator set’s domain.
...
节点列表，基于输入/输出数据依存关系形成部分排序的计算图，拓扑顺序排列。

initializer

Tensor\[\]

命名张量值的列表。 当 `initializer` 与计算图 `graph`输入名称相同，输入指定一个默认值，否则指定一个常量值。
...
计算图 `graph` 的输入参数，在 `‘initializer.’` 中可能能找到默认的初始化值。
...
字符串代表的文本数据。 所有字符串均使用UTF-8编码。

Other

bool

布尔值类型，表示的数据只有两个值，通常为 `true` 和 `false`。
...
> 文章同步发于 [github](https://github.com/HarleysZhang/cv_note/blob/master/9-model_deploy/%E6%8E%A8%E7%90%86%E6%A1%86%E6%9E%B6/ONNX%E6%A8%A1%E5%9E%8B%E5%88%86%E6%9E%90%E4%B8%8E%E4%BD%BF%E7%94%A8.md)、[知乎](https://zhuanlan.zhihu.com/p/371177698)，最新版以github为主。  
