---
layout: post
title: "线上Electron应用具备哪些特征？"
date: "2022-11-09T12:44:15.882Z"
---
线上Electron应用具备哪些特征？
===================
...
这个包的导出文件是 index.js，这个文件并没有什么特殊的逻辑，只是返回了上面 electron.exe 的路径。如果你还记得我们在“如何开发 Vite3 插件构建 Electron 开发环境”章节中介绍的知识，我们就是通过`require("electron")`获取 electron.exe 的路径的，如下代码所示：
...
*   镜像部分：[https://github.com/electron/electron/releases/download/](https://link.zhihu.com/?target=https%3A//github.com/electron/electron/releases/download/)。
*   版本部分：v11.1.0/。
*   文件部分：electron-v11.1.0-win32-x64.zip。

这三部分联合起来最终构成了下载地址，每个部分都有其默认值，也有对应的重写该部分值的环境变量。

*   镜像部分的环境变量：ELECTRON\_MIRROR。
*   版本部分的环境变量：ELECTRON\_CUSTOM\_DIR。
*   文件部分的环境变量：ELECTRON\_CUSTOM\_FILENAME。

如果你因为网络环境问题而无法成功安装 Electron npm 包，那么可以尝试设置 ELECTRON\_MIRROR 的环境变量为[https://npm.taobao.org/mirrors/electron/](https://link.zhihu.com/?target=https%3A//npm.taobao.org/mirrors/electron/)，这是阿里巴巴团队为国内开发者提供的镜像地址。
...
开发者可以通过设置名为 electron\_config\_cache 的环境变量来自定义缓存目录。知道了缓存目录的位置之后，开发者就可以先手动把 Electron 可执行文件及其二进制资源压缩包和哈希文件放置到相应的缓存目录中。这样再通过 npm install 命令安装 Electron 依赖包时，就会先从你的缓存目录里获取相应的文件，而不是去网络上下载了。这对于工作在无外网环境下的开发者来说，无疑是一种非常有价值的手段。
...
当开发者在 64 位操作系统上打 32 位的应用程序安装包时，electron-builder 会去服务器下载 32 位的 Electron 二进制包，从而完成**交叉编译**的需求，这实际上这并不是真正的交叉编译。
