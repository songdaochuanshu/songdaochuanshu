...
通过 SSH 连接到 SDDC Manager 虚拟机，SoS 使用程序可以在如下图中的位置（/opt/vmware/sddc-support）找到它，你可以使用 `--version` 或 `-v` 命令选项查看当前 SoS 实用程序的版本。注意，使用 SoS 程序在执行某些功能选项的时候，可能需要 root 用户才能运行。
