...
**拓展方式注入**  
Spring 给我们提供了两种拓展方式来实现依赖注入，分别是`p 命名空间和c命名空间`，其中p命名空间可以代替属性注入，c命名空间可以替代构造函数注入。

使用两种拓展方式之前，我们需要在 xml 配置文件引入两者的约束，其约束如下：
