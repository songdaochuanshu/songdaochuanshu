...
　　我们进行的主要实验是比较该模型在不同STFT帧长$M$的性能，在感知和客观测量方面。由于我们考虑的模型包括相位和幅度的显式估计，我们还能够分析和量化幅度和相位估计的相对贡献，再次作为帧长的函数。这种分析的方式与\[4\]、\[28\]、\[29\]中的感知实验类似，尽管这里我们使用纯净幅度和相位的估计，而不是纯净或噪声信号。对于每个帧长，我们产生三个纯净语音信号的估计：网络的实际输出以及两个由估计幅度和噪声相位组成的合成信号，反之亦然：
