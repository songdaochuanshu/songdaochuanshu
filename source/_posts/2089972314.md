...
本文介绍为什么linux实时任务不能直接调用`printf()`，首先简单介绍一下终端输出原理，然后如何实现终端输出不影响实时任务实时性给出一个方案，最后介绍xenomai中是如何做到完美`printf()`的。
...
以上就是一个实时linux下开发实时应用程序，由一个普普通通的`printf()`引发的实时性能问题解决，可以看出不起眼的`printf()`要做好远比我们想象的复杂，做底层就是这样，得坐冷板凳耐得住寂寞。几句话共勉：  
"万丈高楼平地起，勿在浮沙筑高台"。  
