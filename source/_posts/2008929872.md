---
layout: post
title: "\"\\"\\\"Chrome扩展的核心：manifest 文件（中）\\\"\\"\""
date: "2023-12-15T01:02:01.326Z"
---
Chrome扩展的核心：manifest 文件（中）
==========================

![Chrome扩展的核心：manifest 文件（中）](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140732671-260013876.png) 在 Chrome 扩展开发中，manifest 文件是项目的核心，其中 "content\_scripts"，"background"，"permissions" 配置项又至关重要！

大家好，我是 dom 哥。我正在写关于 Chrome 扩展开发的系列文章，感兴趣的可以 [点个小星星 ![](https://img.shields.io/github/stars/dom-bro/chrome-extension-development?label=&style=social)](https://github.com/dom-bro/chrome-extension-development) 。

在[上一篇](https://www.cnblogs.com/iovec/p/manifest.html)中已经完成了 Chrome 扩展的雏形，本篇接着介绍 manifest 中的可选字段，完善扩展的细节。
...
**向 web 页面注入 JavaScript 和 CSS**。可以说这是 Chrome 扩展的灵魂。当指定 content\_scripts 后，每当页面加载时，content\_scripts 也将随之加载。
...
*   `"run_at"`：**脚本注入时机**。默认情况下，扩展会在页面处于空闲状态时注入 css 和 js。有以下3个时机可供选择：
    ...
    *   `document_idle` 当页面空闲时注入。这也是默认的注入时机！浏览器会在 `document_end` 和 `window.onload` 触发前这个时间段内选择时机注入，具体注入时机取决于页面的复杂程度。
    ...
    ![](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140315703-1438533886.png)
...
*   `"world"`：**content-script 的脚本执行环境**。默认情况下，content-script 运行在一个隔离的沙箱环境中。这意味着和页面的运行时环境是隔离开来的两个独立环境，最明显的表现就是两个环境里的 全局变量 `window` 不通。该字段有以下2个选择：
    ...
    *   `MAIN` 页面运行时环境。这意味者扩展脚本和页面脚本共享运行时，同一个 window，同一个世界，同一个梦想。
    ...
    ![](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140253644-1912844574.png)
    
    可以发现 `ISOLATED` 隔离环境访问不到 `window.customVariable`，值是 undefined，而 `MAIN` 环境则能访问到！
    
    我画了一张图，以便直观的展示 `ISOLATED` 和 `MAIN` 执行环境的区别
    
    ![](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140232295-731397186.png)
...
作为后台服务，service worker 的主要用途是注册事件监听，响应各种事件，比如网络请求，content\_scripts 传来的 message 等等。Chrome 扩展提供了大量 chrome.\* 接口可供使用，这些接口大多都能在 `"background"` 中使用，但 `"content_scripts"` 只能使用其中的少数，这是为安全起见故意设计的。
...
![](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140152071-312129775.jpg)

当 service worker 闲置时，它会自动休眠，显示为**无效**。如下图所示：

![background-dim.jpg](https://img2023.cnblogs.com/blog/882926/202312/882926-20231214140101534-1097453599.jpg)
...
觉得不错可以 [点个小星星 ![](https://img.shields.io/github/stars/dom-bro/chrome-extension-development?label=&style=social)](https://github.com/dom-bro/chrome-extension-development) 支持一下🌹
