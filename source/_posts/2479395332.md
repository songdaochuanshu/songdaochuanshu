...
this显示绑定，顾名思义，它有别于this的隐式绑定，而隐式绑定必须要求一个对象内部包含一个指向某个函数的属性（或者某个对象或者上下文包含一个函数调用位置），并通过这个属性间接调用这个函数，从而把this简介/隐式绑定到这个对象上。但是this的隐式绑定存在一个**绑定对象丢失**问题，如下面代码所示：
