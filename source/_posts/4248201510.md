...
这个value-1的结果要么是1，要么是5，为啥？仔细分析下。一般的来说，base也就是函数所在的内存的起始地址末尾两字节一般都是 00 00。也就是说 00007FFB098C5EC0 - base 的结果一定四0xnnnnnnnnnnnn5EC0。n表示未知数。因为上面的公式&31，所以只需要关注最后两个字节就可以了。
