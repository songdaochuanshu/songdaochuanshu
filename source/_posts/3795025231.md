---
layout: post
title: "netty系列之:netty对marshalling的支持"
date: "2022-05-20T10:22:16.937Z"
---
...
*   [简介](#简介)
*   [netty中的marshalling provider](#netty中的marshalling-provider)
*   [Marshalling编码器](#marshalling编码器)
*   [Marshalling编码的另外一种实现](#marshalling编码的另外一种实现)
*   [总结](#总结)
...
是将Object对象编码成为ByteBuf。回顾一下之前我们讲到的通常对象的编码都需要用到一个对象长度的字段，用来分割对象的数据，同样的MarshallingEncoder也提供了一个4个字节的LENGTH\_PLACEHOLDER，用来存储对象的长度。
...
encode的逻辑很简单，首先从provider中拿到marshaller对象，然后先向out中写入4个字节的LENGTH\_PLACEHOLDER，接着使用marshaller向  
...
> 本文已收录于 [http://www.flydean.com/17-1-netty-marshalling/](http://www.flydean.com/17-1-netty-marshalling/)
> 
