---
layout: post
title: "\"\\"\\\"【OpenVINO™】在 MacOS 上使用 OpenVINO™ C# API 部署 Yolov5\\\"\\"\""
date: "2024-02-07T00:56:00.637Z"
---
【OpenVINO™】在 MacOS 上使用 OpenVINO™ C# API 部署 Yolov5
=================================================

![【OpenVINO™】在 MacOS 上使用 OpenVINO™ C# API 部署 Yolov5](https://img2024.cnblogs.com/blog/2933426/202402/2933426-20240206140815543-410076662.png) YOLOv5 是革命性的 "单阶段"对象检测模型的第五次迭代，旨在实时提供高速、高精度的结果，是世界上最受欢迎的视觉人工智能模型，代表了Ultralytics对未来视觉人工智能方法的开源研究，融合了数千小时研发中积累的经验教训和最佳实践。同时官方发布的模型已经支持 OpenVINO™ 部署工具加速模型推理，因此在该项目中，我们将结合之前开发的 OpenVINO™ C# API 部署 YOLOv5 DET 模型实现物体对象检测。
...
*   [1\. 前言](#1-前言)
    *   [1.1 OpenVINO™ C# API](#11-openvino-c-api)
    *   [1.2 YOLOv5](#12-yolov5)
*   [2\. 模型下载与转换](#2-模型下载与转换)
    *   [2.1 环境安装](#21-环境安装)
    *   [2.2 Yolov5 模型下载](#22-yolov5-模型下载)
    *   [2.3 转换IR模型](#23-转换ir模型)
*   [3\. Yolov5 DET 项目配置(OpenCvSharp版)](#3-yolov5-det-项目配置opencvsharp版)
    *   [3.1 项目创建](#31-项目创建)
    *   [3.2 添加项目依赖](#32-添加项目依赖)
    *   [3.3 定义预测方法](#33-定义预测方法)
        *   [(1) 使用常规方式部署模型](#1-使用常规方式部署模型)
        *   [(2) 使用模型结构处理处理数据](#2-使用模型结构处理处理数据)
        *   [(3) 使用 OpenVINO™ C# API 封装的接口](#3-使用-openvino-c-api-封装的接口)
    *   [3.4 预测方法调用](#34-预测方法调用)
*   [4\. Yolov5 DET 项目配置(Emgu.CV 版)](#4-yolov5-det-项目配置emgucv-版)
    *   [4.1 添加项目依赖](#41-添加项目依赖)
    *   [4.2 定义预测方法](#42-定义预测方法)
*   [5\. 项目运行与演示](#5-项目运行与演示)
    *   [5.1 项目编译](#51-项目编译)
    *   [5.2 项目文件运行](#52-项目文件运行)
*   [6\. 总结](#6-总结)
...
![](https://s2.loli.net/2024/01/18/vc5VeJ2BQknhitp.png)
...
![](https://s2.loli.net/2024/01/21/AlOgk92WGQBXMJ6.png)
...
![](https://s2.loli.net/2024/02/01/HJmswpdlhBKiE8P.png)

使用Netron工具打开模型文件，查看模型结构，如下图所示：

![](https://s2.loli.net/2024/02/01/ERklz2bQymIAr6Y.png)
...
![](https://s2.loli.net/2024/02/01/Cr5UKHaQjkqwcz8.png)
...
![](https://s2.loli.net/2024/02/01/NvJaTo5XwG6YI7g.png)
...
关于在MacOS上搭建 OpenVINO™ C# API 开发环境请参考以下文章： [在MacOS上搭建OpenVINO™C#开发环境](..%5Cinatall%5CInstall_OpenVINO_CSharp_MacOS_cn.md)
...
![](https://s2.loli.net/2024/02/01/gejy9DoWSRZ7YdO.png)
...
![](https://s2.loli.net/2024/02/01/lGRr1diWtHn6pYk.png)

![](https://s2.loli.net/2024/02/01/wPvx5W1HQdNynrR.jpg)
