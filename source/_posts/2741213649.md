---
layout: post
title: "\"仿照AirDrop(隔空投送)优雅地在局域网中传输文件\""
date: "2024-01-02T00:59:25.652Z"
---
...
通过这种方式，任何拥有浏览器的设备都有传输数据的可能，不需要借助数据线传输文件，也不会受限于Apple全家桶才能使用的隔空投送，并且在实现的过程中我还拓展了多文件发送、文本消息、尝试公网连接等能力，总结起来通过这种方式我们可以获得如下的收益：
...
在耗费了两个周末的时间之后，将整个功能完善了出来 [Github](https://github.com/WindrunnerMax/FileTransfer)，并且可以尝试使用在线 [DEMO](https://webrtc.touchczy.top/)，因为本身数据不会经由服务器转发，所以部署后大概占用`20MB`左右的内存，使用效果可以查看`README`的 [视频](https://github.com/WindrunnerMax/FileTransfer?tab=readme-ov-file#filetransfer)。
