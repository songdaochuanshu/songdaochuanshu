...
\\\[\\left|\\psi\\right>=\\psi(x,t)=Ae^{kx-\\omega t+\\phi\_0}=Acos(kx-\\omega t+\\phi\_0)+iAsin(kx-\\omega t+\\phi\_0) \\\]
...
这两个参数分别所表征的物理含义为：电子从q轨道跃迁到p轨道的动能和原子核势能增减总和，在紧束缚模型下又称为hopping energy（电子跳跃能），以及两个电子分别从r、s轨道跃迁到q、p轨道的电子之间相互作用势能的变化（单体动能和原子核势能在前面一项中已经考虑到了）。关于对位置的积分，可以理解为给定的轨道占据一定的空间，空间上的任一位置的跃迁都被认为是该轨道所发生的跃迁，因此要遍历轨道所占据的空间。如此一来，我们就可以**将一个量子比特定义成一个电子轨道，从而实现在量子计算机上面完成量子化学计算的任务**。
