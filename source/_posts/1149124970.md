---
layout: post
title: "深入剖析（JDK）ArrayQueue源码"
date: "2022-07-18T09:18:39.536Z"
---
深入剖析（JDK）ArrayQueue源码
=====================

![深入剖析（JDK）ArrayQueue源码](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182321996-744000889.png) 本篇文章主要跟大家介绍JDK容器ArrayQueue内部的实现原理以及他的源代码，各种操作直接用图描述，清晰明了！
...
![](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182204052-737476335.png)

因为是初始状态`head`和`tail`的值都等于0，指向数组当中第一个数据。现在我们向`ArrayQueue`内部加入5个数据，那么他的内存布局将如下图所示：

![](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182213493-2043094524.png)

现在我们删除4个数据，那么上图经过4次删除操作之后，`ArrayQueue`内部数据布局如下：

![](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182222001-292800751.png)

在上面的状态下，我们继续加入8个数据，那么布局情况如下：  
![](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182230363-86281296.png)
...
在`resize`函数当中首先申请新长度的数组空间，然后将原数组的数据一个一个的拷贝到新的数组当中，注意在这个拷贝的过程当中，重新更新了`head`与`tail`，而且并不是简单的数组拷贝，因为在之前的操作当中`head`可能已经不是了0，因此新的拷贝需要我们一个一个的从就数组拿出来，然后放到新数组当中。下图可以很直观的看出这个过程：

![](https://img2022.cnblogs.com/blog/2519003/202207/2519003-20220717182146489-1049535344.png)
