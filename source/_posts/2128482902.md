---
layout: post
title: "MM32F0140 UART1硬件自动波特率校准功能的使用"
date: "2022-03-30T19:15:24.245Z"
---
...
![](https://img2022.cnblogs.com/blog/2342755/202203/2342755-20220330175451031-1004891136.jpg)
...
void Bsp\_UART\_SendBytes(UART\_TypeDef\* uart,u8\* pbuf, u16 len)
{
    while(len--)
    {
        Bsp\_UART\_SendByte(uart,\*pbuf++);
    }
}
...
void Bsp\_UART\_SendString(UART\_TypeDef\* uart,char\* str)
{
    while(\*str)
    {
        Bsp\_UART\_SendByte(uart,(u8)(\*str++));
    }
}
...
{   
    //UART1 Rx Flag
    if(gUART1\_Rx\_Flag == true)
    {
        gUART1\_Rx\_Flag \= false;
        
        //Send the received data as-is, for example：0x55 0x55 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09
        Bsp\_UART\_SendBytes(UART1,gUART1\_Rx\_Buf, gUART1\_Rx\_Cnt); 
        
        //Receive data from the serial port of the host computer：0xF8 0x55 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09
        if((gUART1\_Rx\_Buf\[0\] == 0xF8) && (gUART1\_Rx\_Buf\[1\] == 0x55))
        {
            LED1\_TOGGLE();     
        }  
        //Receive data from the serial port of the host computer：0xF8 0x01 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09
        else if((gUART1\_Rx\_Buf\[0\] == 0xF8) && (gUART1\_Rx\_Buf\[1\] == 0x01))
        {
            LED1\_TOGGLE();   
        } 
        //Receive data from the serial port of the host computer：0xF8 0x02 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09
        else if((gUART1\_Rx\_Buf\[0\] == 0xF8) && (gUART1\_Rx\_Buf\[1\] == 0x02))
        {
            LED1\_TOGGLE();
        } 
        //Receive data from the serial port of the host computer：0xF8 0x03 0x02 0x03 0x04 0x05 0x06 0x07 0x08 0x09
        else if((gUART1\_Rx\_Buf\[0\] == 0xF8) && (gUART1\_Rx\_Buf\[1\] == 0x03))
        {
            LED1\_TOGGLE();    
        } 
...
![](https://img2022.cnblogs.com/blog/2342755/202203/2342755-20220330190107596-1234914691.jpg)

图1 如上串口助手显示MM32F0140的UART1已自动检测到19200波特率并打印出收到的正确数据

![](https://img2022.cnblogs.com/blog/2342755/202203/2342755-20220330190331339-1025700308.jpg)

图2 如上串口助手显示MM32F0140的UART1已自动检测到38400波特率并打印出收到的正确数据

![](https://img2022.cnblogs.com/blog/2342755/202203/2342755-20220330190415416-2101891257.jpg)

图3 如上串口助手显示MM32F0140的UART1已自动检测到57600波特率并打印出收到的正确数据

![](https://img2022.cnblogs.com/blog/2342755/202203/2342755-20220330190447195-648714511.jpg)
