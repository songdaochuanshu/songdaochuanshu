...
同样，\\(v\_i\\)和\\(v\_j\\)可能由他们的几个共同朋友介绍，因此，我们的下一个目标是联合建模多个开放三元组（具有一对共同的未链接顶点）是如何演化的。首先定义集合\\(B^t(i,j)\\)为\\(v\_i\\)和\\(v\_j\\)在\\(t\\)时间的共同邻居，并且定义一个向量\\(\\boldsymbol{\\alpha}^{t, i, j}=\\left(\\alpha\_{k}^{t, i, j}\\right)\_{k \\in B^{t}(i, j)}\\),其中对于每一维的值如下定义，如果\\(t+1\\)处的开放三元组\\((v\_i，v\_j，v\_k)\\)将发展为闭合三元组。换句话说，在\\(v\_k\\)的影响下，vi和vj将成为朋友，则为1。
