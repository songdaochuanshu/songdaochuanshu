...
所有的发布器都在publisher目录下，在下面你可以找到segmentfault\_publisher这个专门给segmentfault的发布器。
...
所以我们可以直接使用find\_element by ID直接获得title的input，然后调用send\_keys方法，把title的内容输入进去。
