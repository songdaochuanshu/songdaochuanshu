...
void NFA::visit4p(const State\* st, FILE\* fp) {
  ...
  for (int i = 0; i < 2; i++) {
    if (State\* p = st->next\[i\]) {
      ...
      fprintf(fp, "%d \[shape=circle\]\\n", st->id, p->id, label);
      // DOT支持不带BOM的UTF-8编码的文件。ε的UTF-8编码是\\xce\\xb5
      if (st->ch == EPSILON) strcpy(label, "\\xce\\xb5");
      fprintf(fp, "%d -> %d \[label = <%s>\]\\n", st->id, p->id, label);
      visit4p(p, fp);
    }
  }
...
![](https://img2023.cnblogs.com/blog/2606546/202212/2606546-20221222220905544-1928157951.png)