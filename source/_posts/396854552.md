...
print和match都是递归遍历图。print把visited去掉可能陷入无限递归。match可以拽下名词：guided visit.

*   [编译原理之正则表达式转NFA - ChrisZZ](https://www.cnblogs.com/zjutzz/p/3538093.html)
...
void NFA::print(const char\* file\_name) { // 同时输出到屏幕和DOT文件
  ...
  fprintf(fp, "%d \[shape=circle peripheries=2\]\\n", end->id);
  memset(State::\_visited, 0, sizeof(State::\_visited)), visit4p(start, fp);
  fputs("}", fp), fclose(fp);
}
