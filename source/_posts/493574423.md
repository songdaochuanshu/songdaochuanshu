...
求和（或积分）项\\(\\sum\_{\\mathbf{z}}p\_{\\boldsymbol{\\theta}}(\\mathbf{z})p\_{\\boldsymbol{\\theta}}(\\mathbf{x}|\\mathbf{z})\\)常常难以计算，而且\\(\\mathbf{z}\\)的后验分布\\(p\_{\\boldsymbol{\\theta}}(\\mathbf{z}|\\mathbf{x})=p\_{\\boldsymbol{\\theta}}(\\mathbf{x} \\mid \\mathbf{z}) p\_{\\boldsymbol{\\theta}}(\\mathbf{z}) / p\_{\\boldsymbol{\\theta}}(\\mathbf{x})\\)也难以计算，导致EM算法不能使用。
