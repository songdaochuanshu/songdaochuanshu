---
layout: post
title: "女朋友说：你要搞懂了MySQL三大日志，我就让你嘿嘿嘿！"
date: "2022-06-30T17:27:30.832Z"
---
女朋友说：你要搞懂了MySQL三大日志，我就让你嘿嘿嘿！
============================

![女朋友说：你要搞懂了MySQL三大日志，我就让你嘿嘿嘿！](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230748952-162294010.png) MySQL实现事务、崩溃恢复、集群的主从复制，底层都离不开日志，所以日志是MySQL的精华所在。只有了解MySQL日志，才算是彻底搞懂MySQL。 今天一灯就带你深入浅出的学习MySQL的三大日志系统，Redo Log（重做日志）、Undo Log（恢复日志）、Bin Log（备份日志）。
...
![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230424518-1069418359.png)
...
![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230438797-94223805.png)

具体什么时候可以把**Redo Log Buffer**刷到**Redo Log FIle**中，可以通过**innodb\_flush\_log\_at\_trx\_commit**参数配置决定。
...
提交事务后，不会立即刷到OS Buffer中，而是等一秒后刷新到OS Buffer并调用fsync()写入**Redo Log FIle**，可能会丢失一秒钟的数据。

1（实时写

每次提交事务，都会刷新到OS Buffer并调用fsync()写到**Redo Log FIle**，性能较差

2（延迟刷新）

每次提交事务只刷新到OS Buffer，一秒后再调用fsync()写入**Redo Log FIle**。
...
![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230454217-1597872820.png)
...
实现方式通过两个隐藏列trx\_id（最近一次提交事务的ID）和roll\_pointer（上个版本的地址），建立一个版本链。并在事务中读取的时候生成一个ReadView（读视图），在Read Committed隔离级别下，每次读取都会生成一个读视图，而在Repeatable Read隔离级别下，只会在第一次读取时生成一个读视图。

![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230506275-219885709.png)
...
**Bin Log**共有三种日志格式，可以**binlog\_format**配置参数指定。
...
至于什么时候刷新到磁盘，可以**sync\_binlog**配置参数指定。
...
![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230519583-416311739.png)
...
![](https://img2022.cnblogs.com/blog/734446/202206/734446-20220630230529798-502193883.png)
