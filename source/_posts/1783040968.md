...
> 如果第⼆个操作为 volatile 写，不管第⼀个操作是什么，都不能重排序，这就确保了 volatile 写之前的操作不会被重排序到 volatile 写之后。
> 
> 如果第⼀个操作为 volatile 读，不管第⼆个操作是什么，都不能重排序，这确保了 volatile 读之后的操作不会被重排序到 volatile 读之前。
