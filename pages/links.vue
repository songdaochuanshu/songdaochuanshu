  ...
  <div class="min-h-screen bg-gray-50 dark:bg-gray-900">
    ...
    <main class="pt-24 pb-16 px-4">
      <div class="max-w-4xl mx-auto">
        ...
        <div v-if="loading" class="text-center py-12"><div class="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-600 mx-auto"></div><p class="mt-4 text-gray-500 dark:text-gray-400">加载中...</p></div>
        <div v-else-if="error" class="text-center py-12"><p class="text-red-500 dark:text-red-400">{{ error }}</p><button @click="fetchLinks" class="mt-4 px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">重试</button></div>
        <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          <a v-for="link in filteredLinks" :key="link.url" :href="link.url" target="_blank" rel="noopener noreferrer" class="group block bg-white dark:bg-gray-800 rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden border border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-blue-600">
            <div class="p-6"><div class="flex items-start gap-4"><img :src="link.avatar" :alt="link.name" class="w-12 h-12 rounded-full bg-gray-200 dark:bg-gray-700 flex-shrink-0" @error="handleAvatarError" /><div class="flex-1 min-w-0"><h3 class="font-semibold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors truncate">{{ link.name }}</h3><p class="text-sm text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{{ link.description }}</p><span v-if="link.category" class="inline-block mt-2 px-2 py-0.5 text-xs font-medium bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400 rounded">{{ link.category }}</span></div></div></div>
          </a>
        </div>
        <div v-if="!loading && !error && filteredLinks.length === 0" class="text-center py-12"><p class="text-gray-500 dark:text-gray-400">暂无友链</p></div>
      </div>
    </main>
    <footer class="py-8 text-center text-sm text-gray-500 dark:text-gray-400 border-t border-gray-200 dark:border-gray-700"><p>© 2024 松岛川树. All rights reserved.</p></footer>
  </div>
</template>
