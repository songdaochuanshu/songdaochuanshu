...
[bombardier](https://github.com/codesenberg/bombardier) 是一个 HTTP(S) 基准测试工具。它是用 Go 编程语言编写的，并使用优秀的fasthttp代替 Go 的默认 http 库，因为它具有闪电般的快速性能，[详细文档查看](https://pkg.go.dev/github.com/codesenberg/bombardier?utm_source=godoc)
...
6.  通过JObject.Parse解析指标，最后通过BenchmarksEventSource存储并输出到控制台或数据库、csv、json中
...
*   presetHeaders: 预设header，根据全局参数headers，自选其一即可，选择json，那请求的header即为: --header "Accept: application/json,text/html;q=0.9,application/xhtml+xml;q=0.9,application/xml;q=0.8,_/_;q=0.7" --header "Connection: keep-alive"
...
源码地址：[https://github.com/doddgu/crank/tree/sample](https://github.com/doddgu/crank/tree/sample)
