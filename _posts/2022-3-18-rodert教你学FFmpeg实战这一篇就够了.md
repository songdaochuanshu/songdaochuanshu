...
pdf阅读版：  
链接：[https://pan.baidu.com/s/11kIaq5V6A\_pFX3yVoTUvzA](https://pan.baidu.com/s/11kIaq5V6A_pFX3yVoTUvzA)  
提取码：java
...
*   [rodert教你学FFmpeg实战这一篇就够了](#rodert教你学ffmpeg实战这一篇就够了)
*   [前言](#前言)
*   [1\. 简介](#1-简介)
*   [2\. 安装](#2-安装)
...
*   [3\. 命令行操使用](#3-命令行操使用)
    *   *   [3.1.简单说明](#31简单说明)
        ...
        *   [3.5.提取音频](#35提取音频)
        *   [3.6.码率控制](#36码率控制)
        *   [3.7. 视频编码格式转换](#37-视频编码格式转换)
        *   [3.8. 只提取视频ES数据](#38-只提取视频es数据)
        *   [3.9.过滤器的使用](#39过滤器的使用)
            *   [3.9.1. 将输入的1920x1080缩小到960x540输出](#391-将输入的1920x1080缩小到960x540输出)
            *   [3.9.2.视频添加logo](#392视频添加logo)
            *   [3.9.3.去掉视频的logo](#393去掉视频的logo)
        *   [3.10.抓取视频的一些帧，存为jpeg图片](#310抓取视频的一些帧存为jpeg图片)
        *   [3.11.查询操作](#311查询操作)
            *   [3.11.1.查询视频总帧数](#3111查询视频总帧数)
            *   [3.11.2.查询FFmpeg软件信息](#3112查询ffmpeg软件信息)
        *   [3.12.录制](#312录制)
        *   [3.13.直播相关](#313直播相关)
...
官网地址：[https://trac.ffmpeg.org/wiki](https://trac.ffmpeg.org/wiki)
...
> wget [http://ffmpeg.org/releases/ffmpeg-3.4.1.tar.bz2](http://ffmpeg.org/releases/ffmpeg-3.4.1.tar.bz2)
...
> export FFMPEG\_HOME=/usr/local/ffmpeg

> export PATH=$FFMPEG\_HOME/bin:$PATH
...
更多：[http://www.360doc.com/content/13/0829/15/13084517\_310733557.shtml](http://www.360doc.com/content/13/0829/15/13084517_310733557.shtml)
...
> ffmpeg -i input.mp4 -vf "select=between(n,84,208)\*not(mod(n,4))" -vsync 0 image\_%05d.jpg
...
> ffprobe -v error -count\_frames -select\_streams v:0 -show\_entries stream=nb\_read\_frames -of default=nokey=1:noprint\_wrappers=1 input.mp4

说明：读取帧数需要文件解码，时长取决于文件大小

> \-v error：这隐藏了“info”输出(版本信息等)，使解析更容易。  
> \-count\_frames：计算每个流的帧数，并在相应的流部分中报告。  
> \-select\_streams v:0 ：仅选择视频流。  
> \-show\_entries stream = nb\_read\_frames ：只显示读取的帧数。  
> \-of default = nokey = 1：noprint\_wrappers = 1 ：将输出格式(也称为“writer”)设置为默认值，不打印每个字段的键(nokey = 1)，不打印节头和页脚(noprint\_wrappers = 1)。
