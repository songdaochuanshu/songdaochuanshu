...
*   Python 3.9+
*   NVIDIA NIM API Key（[免费注册](https://build.nvidia.com/)）
*   稳定的网络连接
...
Embedding 维度，nv-embed-v1 固定为 4096

`embedding.model`

推荐使用 `nvidia/nv-embed-v1`（对称模型，不需要 input\_type 参数）

`vlm.model`

用于生成摘要的语言模型，推荐 `meta/llama-3.3-70b-instruct`
...
### Q: NVIDIA API 报错 input\_type required
...
**解决**: 使用对称模型 `nvidia/nv-embed-v1`，不需要 input\_type。
...
*   OpenViking 官网: [https://www.openviking.ai](https://www.openviking.ai)
...
*   NVIDIA API 文档: [https://docs.api.nvidia.com/nim/](https://docs.api.nvidia.com/nim/)

* * *

_本教程基于 OpenViking 0.1.17 和 NVIDIA NIM API 测试通过。_