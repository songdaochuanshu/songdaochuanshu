...
gt-checksum v4.0.0 新功能解读（2）：自定义数据类型映射 ✅ 新增 dTypeMappingFile 参数，支持 YAML/JSON 自定义类型映射规则，异构迁移不再手工逐列核对 ✅ 覆盖 Oracle → MySQL、MariaDB → MySQL、MySQL 跨版本升级三大场景，规则自动分组匹配 ✅ 支持 schema/table/column 三级对象匹配 + 条件表达式引擎，精度、小数位、UNSIGNED 等属性精细控制 ✅ First-Match 匹配语义，具体规则优先、通用规则兜底，映射逻辑清晰可控 ✅ nullable/unsigned/autoinc/default 属性覆盖，修复 SQL 自动生成时同步应用 ✅ --preview-dtype-mapping 预览模式，不连库即可验证规则，先预览再校验
...
过滤条件：仅匹配源端列是否具有 AUTO\_INCREMENT 属性
...
**2\. target\_type 必须是合法的 MySQL 类型**
