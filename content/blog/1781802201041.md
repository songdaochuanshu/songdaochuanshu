...
![【原创】如何利用网卡TSN硬件特性实现EtherCAT 确定性发帧与 DC 同步？](https://img2024.cnblogs.com/blog/1250138/202606/1250138-20260618222510818-427734612.png) 本文以 Rockchip RK3576/3588 + stmmac + IgH 为例，重点解析 TSN 在 EtherCAT 主站中的应用。针对 EtherCAT 发送抖动受 OS调度制约的痛点，讲清如何用 TSN 的 EST 门控与 Launch Time 把每帧发送时刻下沉到网卡 PHC硬件，实现无抖动发帧，并给出从应用到 MAC 寄存器的完整下发链路；同时以网卡 PHC 为时钟同步根，经 phc2sys 与therCAT DC 报文拉齐系统时钟、PHC、从站 DC，使 TSN 硬件发包与伺服 SYNC0 同源同步。
...
*   [【原创】如何利用TSN硬件特性实现EtherCAT 确定性发帧与 DC 同步？](#原创如何利用tsn硬件特性实现ethercat-确定性发帧与-dc-同步)
    ...
    *   [3\. 当前 IgH 是如何保证网络收发确定性的，有哪些缺点](#3-当前-igh-是如何保证网络收发确定性的有哪些缺点)
        *   [3.1 接管网卡：ecdev 与 ec\_poll](#31-接管网卡ecdev-与-ec_poll)
        *   [3.2 一帧是怎么发出去的、上一帧是怎么收回来的](#32-一帧是怎么发出去的上一帧是怎么收回来的)
        *   [3.3 周期调度：clock\_nanosleep(TIMER\_ABSTIME)](#33-周期调度clock_nanosleeptimer_abstime)
        *   [3.4 缺点](#34-缺点)
    *   [4\. Rockchip 的 TSN 方案：原理、步骤、流程、框架](#4-rockchip-的-tsn-方案原理步骤流程框架)
        ...
        *   [4.4 DC 同步的源码走读：app\_time 怎么流到每一个从站](#44-dc-同步的源码走读app_time-怎么流到每一个从站)
...
> 源码详见[https://gitee.com/wsg1100/ethercat\_igh](https://gitee.com/wsg1100/ethercat_igh) 来源瑞芯微开放SDK，仅供参考
...
*   **T\_往返穿行**随**从站数量**和**线缆长度**线性增长。线缆约 5 ns/m；每个从站 ESC 百纳秒量级。50 台从站 + 几十米线，往返就可能占到十几到几十微秒。
*   **数据量**：一帧装多少数据报受以太网帧长限制，从站越多、PDO 越大，单帧越大，组帧和 DMA 处理越久；装不下还要分多帧。
*   **T\_软件处理发送**受 OS 抖动影响。
*   **T\_余量**是吸收抖动的安全垫。**OS 抖动越大，余量必须越宽，能用的周期就越长（频率越低）。**
...
> 
> ![tsn\_ov](https://wsg-blogs-pic.oss-cn-beijing.aliyuncs.com/xenomai/tsn_ov.png)
...
*   每个表项 `<gate_mask, interval>`：在 `interval` 纳秒内，哪些 TX 队列"开门"（gate\_mask 对应 bit=1）；
...
### 3.3 周期调度：clock\_nanosleep(TIMER\_ABSTIME)

收发靠绕过网络栈，周期节奏靠应用层一个 SCHED\_FIFO 最高优先级、绑核的实时线程：
...
### 4.4 DC 同步的源码走读：app\_time 怎么流到每一个从站
...
那 `app_time` 是怎么流到每一个从站的？分三步，对应三个 API。**第一步，喂 app\_time**：
...
1.  应用 `clock_gettime` 取 `app_time`，调 `ecrt_master_application_time` 存进 `master->app_time`（不发报文）。
...
2.  **开 EST/TBS 时，应用 `app_time` 改用 `CLOCK_REALTIME`**（而非 `CLOCK_MONOTONIC`），这样 `app_time` ≈ PHC 值，EST/TBS 的硬件时刻与 `app_time` 同域、不错位。demo 正是这么做的：`ec_master_test_MADHT1505BA1.c:41-45` 在 `#if (USE_TSN_MODE_EST||USE_TSN_MODE_TBS)` 下把 `CLOCK_TO_USE` 切成 `CLOCK_REALTIME`。不开 EST/TBS 时仍可用 `CLOCK_MONOTONIC`（不会被 NTP 跳变，周期更稳）。
3.  **主站周期性把 `app_time` 写给参考从站**（`sync_reference_clock_to` FPWR `0x0910`），再用 `sync_slave_clocks`（FRMW `0x0910`）把全总线时钟拉齐到参考从站。
...
*   **网卡时钟 ↔主站周期应用调度(系统时钟)↔ （参考时钟从站 ↔ 其他从站）**：网卡stmmac PHC 是独立硬件时钟，由用户态 `ptp4l`/`phc2sys` 将系统时钟`CLOCK_REALTIME`与网卡同步；IgH 显式地用 `ecrt_master_sync_reference_clock_to()` 写 0x0910 寄存器把参考从站 DC 拉到 `CLOCK_REALTIME`，再用 `ecrt_master_sync_slave_clocks()` 做总线内广播对齐。
