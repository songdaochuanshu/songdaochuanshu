...
EF Core + Pomelo MySQL、Redis、CAP、Hangfire
...
自带 Blazor/React 设计器，与 Vue 生态不匹配

原生 Vue 3 + Ant Design Vue

数据库

默认 SQLite，MySQL 支持需额外配置

复用现有 EF Core + MySQL

.NET 版本

Elsa 3.x 对 .NET 10 的兼容性需验证
...
![流程定义列表与编辑](https://img2024.cnblogs.com/blog/553504/202602/553504-20260206174444280-259695173.png)

_流程定义列表：可新增、编辑、发布、归档；编辑时在下方进行流程节点设计。_
...
![我的待办](https://img2024.cnblogs.com/blog/553504/202602/553504-20260206174503542-837619307.png)

_我的待办：审批人在此处理待审批任务。_
...
![用户管理-提交审批](https://img2024.cnblogs.com/blog/553504/202602/553504-20260206174554266-1617952302.png)

_新增用户时可选择「提交审批」，进入已配置的用户管理审批流程。_
...
`WorkflowDefinitionId`、`WorkflowInstanceId` 避免 ID 误用

值对象

`WorkflowNode` 作为 `WorkflowDefinition` 的子实体集合
